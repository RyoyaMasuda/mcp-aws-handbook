...
def main():
    ...
    aws_api_mcp_client = create_stdio_mcp_client(
        command="uvx",  
        args=["awslabs.aws-api-mcp-server@1.3.3"],
        env={
    ...
    with gateway_server_client, aws_api_mcp_client:
        tools = gateway_server_client.list_tools_sync()
        tools.extend(aws_api_mcp_client.list_tools_sync())
        # Bedrockのモデルを定義
