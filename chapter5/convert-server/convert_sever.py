...
# MCPサーバーの初期化
mcp = FastMCP("DocumentConvertServer", host="0.0.0.0", stateless_http=True)
...
async def convert_to_pptx(text: str = Field(description="pptxに変換するテキスト")) -> str:
    ...
    # Marpでpptxファイルを生成
    response = await _process_marp(TEMP_FILE_PATH, output_file_path)
   
    # 生成されたファイルをS3にアップロードし、URLを取得
    url = _upload_to_s3(output_file_path, BUCKET, output_file_path)
