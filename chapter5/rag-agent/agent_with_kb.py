...
class RagAgent:
    def __init__(self):
        ...
        self.aws_kb_mcp_client = self.create_stdio_mcp_client(
            ...
            args=["awslabs.bedrock-kb-retrieval-mcp-server@1.0.12"],  # AWS Labs提供のKB検索MCPサーバー
            env={
                "AWS_ACCESS_KEY_ID": os.getenv("AWS_ACCESS_KEY_ID"),
                "AWS_SECRET_ACCESS_KEY": os.getenv("AWS_SECRET_ACCESS_KEY"),
    ...
    def create_agent(self, tools: list):
        ...
        return Agent(
            ...
            system_prompt="AWSに関する質問はAWS MCPサーバを用いて、企業情報についてはBedrock Knowledge Base MCPサーバーを用いて回答してください。その参考先も定義してください。",
            tools=tools,
